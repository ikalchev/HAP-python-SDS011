...
from sds011 import SDS011 as AirSensor
...
class SDS011(Accessory):
    ...
    def __init__(self, serial_port, *args, calib_duration_s=15, **kwargs):
    ...
    @Accessory.run_at_interval(SLEEP_DURATION_S)
    async def run(self):
        """Start updating the air quality readings.

        Wake the sensor, wait for `calib_duration_s` seconds for the sensor to calibrate,
        read from the sensor and finaly put it to sleep.
        """
        logger.debug("Waking up sensor.")
        self.sensor.sleep(sleep=False)
        await asyncio.sleep(self.calib_duration_s, loop=self.driver.loop)
